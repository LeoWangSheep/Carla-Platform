...
class CollisionSensor(Sensor):
    ...
    def _on_event(weak_self, event):
        ...
        curr_time = time.time()
        if self._collision_time == 0 or (self._collision_time > 0 and curr_time - self.last_coll_time > 3):
            self._collision_time += 1
            # print("Collision with ", actor_type)
            self._sensor_data_container.update_data(self._sensor_type, self._collision_time, curr_time)
            self.last_coll_time = time.time()
...
class SensorList():
    ...
    def append_collision(self, attach_vehicle):
        sensor_item = {'id': 'Collision'}
        this_sensor = CollisionSensor(self._env, self._sensor_data)
        self._sensor_data.add_sensor(sensor_item)
        this_sensor.attach_to_vehicle(attach_vehicle)
        self._sensor_list.append(this_sensor)
